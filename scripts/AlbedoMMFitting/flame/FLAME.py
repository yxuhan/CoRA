...
class FLAME(nn.Module):
    ...
    def forward(self, shape_params, cameras, trans_params=None, scale_params=None, rot_params=None, neck_pose_params=None, jaw_pose_params=None, eye_pose_params=None, expression_params=None, eyelid_params=None):
        ...
        vertices, _, head_transform = lbs(betas, full_pose, template_vertices,
        ...
        vertices = vertices * scale_params + trans_params.unsqueeze(dim=1)
        lmk68 = lmk68 * scale_params + trans_params.unsqueeze(dim=1)
        mp = mp * scale_params + trans_params.unsqueeze(dim=1)

        act_trans = head_transform[:, :3, -1] * scale_params + trans_params
        return vertices, lmk68, mp, act_trans
...
class AlbedoMMFlameTex(nn.Module):
    def __init__(self, config):
        super(AlbedoMMFlameTex, self).__init__()
        tex_space = np.load(config.tex_space_path)
        # load diffuse albedo
        n_pc = 145
        scale = 255.0
        texture_mean = tex_space["MU"].reshape(1, -1)
        texture_basis = tex_space["PC"].reshape(-1, n_pc)
        n_tex = config.tex_params
        texture_mean = torch.from_numpy(texture_mean).float()[None, ...] * scale
        texture_basis = torch.from_numpy(texture_basis[:, :n_tex]).float()[None, ...] * scale
        self.register_buffer('texture_mean', texture_mean)
        self.register_buffer('texture_basis', texture_basis)
        # load specular albedo
        specular_mean = tex_space["specMU"].reshape(1, -1)
        specular_basis = tex_space["specPC"].reshape(-1, n_pc)
        specular_mean = torch.from_numpy(specular_mean).float()[None, ...] * scale
        specular_basis = torch.from_numpy(specular_basis[:, :n_tex]).float()[None, ...] * scale
        self.register_buffer('specular_mean', specular_mean)
        self.register_buffer('specular_basis', specular_basis)
    ...
    def forward(self, texcode):
        ...
        specular = self.specular_mean + (self.specular_basis * texcode[:, None, :]).sum(-1)
        specular = specular.reshape(texcode.shape[0], 512, 512, 3).permute(0, 3, 1, 2)
        specular = F.interpolate(specular, self.image_size, mode='bilinear')
        specular = specular[:, [2, 1, 0], :, :]
        return texture / 255., specular / 255.
