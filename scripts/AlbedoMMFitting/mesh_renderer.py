...
class MeshRenderer:
    ...
    def render_mesh(self, mesh_dict, cam_int, cam_ext):
        ...
        face_attributes = torch.gather(attributes, dim=1, index=faces)  # [b,3f,c]
        face_attrs = mesh_dict["face_attributes"]  # [b,f,3,c2]
        face_attrs = face_attrs.reshape(b, nf * 3, -1)
        face_attributes = torch.cat([face_attributes, face_attrs], dim=-1)
        face_attributes = face_attributes.reshape(b * nf, 3, -1)  # in pytorch3d, the index of mesh#2's first vertex is set to nface
        output = interpolate_face_attributes(pix_to_face, bary_coords, face_attributes)
        output = output.squeeze(-2).permute(0, 3, 1, 2)
        return output[:, :c], output[:, c:], pix_to_face
